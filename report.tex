...
\subsubsection{Palavras-chave e Tokens Principais}

\begin{tabular}{|l|l|l|}
\hline
\textbf{Categoria} & \textbf{Palavras-chave} & \textbf{Significado} \\
\hline
Tipos & \texttt{void}, \texttt{number}, \texttt{string}, \texttt{bool}, \texttt{list}, \texttt{dict}, \texttt{any} & Especificadores de tipo \\
OO & \texttt{class}, \texttt{extends}, \texttt{new}, \texttt{this}, \texttt{super} & Orientação a objetos \\
Controle & \texttt{if}, \texttt{else}, \texttt{while}, \texttt{for}, \texttt{break}, \texttt{continue}, \texttt{return} & Fluxo de controle \\
Paralelismo & \texttt{par}, \texttt{seq} & Blocos paralelos/sequenciais \\
Canais & \texttt{s\_channel}, \texttt{c\_channel}, \texttt{send}, \texttt{receive} & Comunicação inter-processos \\
Procedural & \texttt{func}, \texttt{var} & Funções e variáveis procedurais \\
E/S & \texttt{println}, \texttt{print} & Saída formatada \\
Literais & \texttt{true}, \texttt{false}, NÚMERO, STRING & Valores constantes \\
\hline
\end{tabular}
...
A análise semântica (executada por \texttt{ms-semantic}) verifica:
\begin{itemize}
    \item Toda classe que estende outra deve referenciar uma superclasse existente.
    \item Nenhuma declaração de classe ou função com nome duplicado é permitida.
    \item Tipos em atribuições devem ser compatíveis (verificação básica; herança permite upcast).
    \item Blocos \texttt{par} executam seus statements concorrentemente (threads no interpretador, sockets em distribuído).
    \item Canais \texttt{s\_channel} são síncronos; \texttt{c\_channel} são assincronos (MVP; suporte completo em Fase 4+).
\end{itemize}
